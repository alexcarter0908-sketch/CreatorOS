$repoRoot = "C:\Users\hp\Downloads\CreatorOS\CreatorOS-main"
$targetFile = Join-Path $repoRoot "frontend\app\settings\page.tsx"

if (Test-Path $targetFile) {
    Copy-Item $targetFile "$targetFile.bak2" -Force
    Write-Host "Backup ban gayi: $targetFile.bak2" -ForegroundColor Yellow
}
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        ...
        <div className="min-w-0 flex-1 space-y-6">
          ...
          {activeTab === "notifications" && (
            <SectionCard icon={Bell} title="Notifications" description="Choose what CreatorOS notifies you about.">
              <div className="mb-4">
                <ComingSoonBadge />
              </div>
              <div className="space-y-3 opacity-60">
                {[
                  "Email me when a generation finishes",
                  "Email me when a YouTube publish completes",
                  "Email me about low credit balance",
                ].map((label) => (
                  <div
                    key={label}
                    className="flex cursor-not-allowed items-center justify-between rounded-xl border border-border px-4 py-3"
                  >
...
Write-Host "File update ho gayi: $targetFile" -ForegroundColor Green
Write-Host "Ab chalao: cd frontend; npm run dev  (ya docker compose up -d --build frontend)" -ForegroundColor Cyan
