...
function CollapsibleText({ text }: { text: string }) {
  ...
  return (
    <div className="my-1 overflow-hidden rounded-lg border border-border bg-muted/50">
      <div
        className={`whitespace-pre-wrap p-3 text-sm leading-relaxed ${expanded ? "" : "max-h-32 overflow-hidden"}`}
      >
        {text}
      </div>
      <button
...
function renderContent(content: string, onCopy: (t: string) => void, collapsible: boolean) {
  return parseSegments(content).map((seg, i) =>
    seg.type === "code" ? (
      ...
      <p key={i} className="whitespace-pre-wrap leading-relaxed">{seg.content}</p>
    )
  );
}
    ...
    const poll = async () => {
      try {
        ...
        if (wf.status === "completed" || wf.status === "failed" || wf.status === "completed_with_errors") {
          ...
          if (needsUpgrade) {
            toast.error("Generation failed - your plan needs more credits.", {
              action: {
                label: "Upgrade",
                onClick: () => router.push("/billing"),
              },
