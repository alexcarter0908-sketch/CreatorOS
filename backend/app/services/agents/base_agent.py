...
RESPONSE_STANDARD = (
    ...
    "  REQUIRED DEPTH (this level of specificity): WhatsApp integration via Meta WhatsApp Cloud API "
    "(official, roughly 0.01 to 0.03 dollars per conversation) or Twilio WhatsApp API (roughly 0.005 "
    "dollars per message plus Twilio fee) - webhook receives incoming client messages, matches against "
    "Property Database by budget and location fields, replies with a template message containing price, "
    "images, and a booking link.\n"
    ...
    "each (for example: developer cost for X weeks, the specific paid API and its monthly cost at N "
    "messages, hosting cost per month) - a single range with no breakdown is a rule violation, not an "
    "acceptable shortcut.\n"
    "- If a critical detail is unknown (their current tools, budget, scale, monthly lead volume) and it "
    "would meaningfully change the recommendation, ask ONE specific question about it instead of "
    "silently guessing a generic answer.\n"
    "- Write like a senior consultant who will be held accountable for this advice actually working - "
